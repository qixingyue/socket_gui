...
$(function(){
	...
	//绑定用户信息
	_n("name").val("<?php echo objpro($capture, "name")?>");
	_n("dbtype").val("<?php echo objpro($capture, "db_type")?>");
	<?php $sourcedb = $capture->source_db; //var_dump($sourcedb);?>
	_n("tns1").val("<?php echo isset($sourcedb->tns_1)? $sourcedb->tns_1 : ""?>");
	_n("tns2").val("<?php echo isset($sourcedb->tns_2)? $sourcedb->tns_2 : ""?>");
	_n("log1").val("<?php echo isset($capture->arch_path1) ? $capture->arch_path1 : ""?>");
	_n("log2").val("<?php echo isset($capture->arch_path2) ? $capture->arch_path2 : ""?>");
	_n("user").val("<?php echo isset($sourcedb->db_user) ? $sourcedb->db_user:""?>");
	_n("loginpassword").val("<?php echo isset($sourcedb->db_password)? $sourcedb->db_password:""?>");

	<?php $downstream_db = $capture->downstream_db; if($downstream_db != NULL && isset($downstream_db->db_user) && $downstream_db->db_user!=""):?>
		_n("offline").attr("checked","checked");
		_n("tns3").val("<?php echo isset($downstream_db->tns)?$downstream_db->tns:""?>");
		_n("db_user").val("<?php echo isset($downstream_db->db_user)?$downstream_db->db_user:""  ?>");
		_n("password2").val("<?php echo isset($downstream_db->db_password)? $sourcedb->db_password : ""?>");
		_n("offline").change();
	<?php endif;?>
   users = <?php
			function toArray($a){
			...
			echo isset($capture->schemas->schema) ? toArray($capture->schemas->schema):"[]"?>;
