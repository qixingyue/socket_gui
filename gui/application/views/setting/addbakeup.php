...
<div id="rightFrame"> <div id="wrap">
   <?php echo form_open();?>
   <div class="pageTitle"><?php echo "数据装载和事务级数据恢复服务>>";?></div>
   <table >
     <tr>
       <td>名称</td>
...
HTML
   ...
   <div id="hiddenChangeBox">
     <input type="checkbox"  id="hiddenCheckBox" name="skiptag" />
   ...
   <table class="mt15" id="backUpTable">
     ...
     <tr>
       <td>备份数据保留期限(天)</td>
       <td><input type="text" style="width: 150px;" name="savedays"/></td>
     </tr>
   </table>
   ...
   <div style="padding-left:150px; padding-bottom:10px; clear:left"> 
     ...
     <input name="cancel" value="取消" type="button" class="buttonSpan" onclick="window.location.hre='<?php echo site_url("setting/editgroup/". $group)?>'"/>
     </span> </div>
     
 </div>
 <?php echo form_hidden("js_hidden_field");?>
 <?php echo form_close();?>
</div><!--end rightFrame -->  
</div>
<!--end content -->
...
function bh_edit(rowid,url){
	_c(function(){
		...
		if(r!=null){
			var row = '<tr rowid="'+rowid+'"><td>'+r.userName+'</td><td>'+r.objtype+'</td><td>'+r.objlist+'</td><td><a href="<?php q_do_nothing()?>" onclick="bh_edit('+rowid+',\''+url+'\')">修改>></a> <a href="<?php q_do_nothing()?>" onclick="pc_del('+rowid+')">删除>></a></td></tr>';
...
function pc_edit(rowid,url){
	_c(function(){
		...
		var r =	 loadFromUrl(url,{"mode":"edit","current":m,"userArguments":loadArguments()});
		if(r!=null){
			var row = '<tr rowid="'+rowid+'"><td>'+r.userName+'</td><td>'+r.objtype+'</td><td>'+r.objlist+'</td><td><a href="<?php q_do_nothing()?>" onclick="pc_edit('+rowid+',\''+url+'\')">修改>></a> <a href="<?php q_do_nothing()?>" onclick="pc_del('+rowid+')">删除>></a></td></tr>';
...
$(document).ready(function(){
	...
	$("form").submit(function(){
		_n("js_hidden_field").val("Hello world!");
		return true;
	});
