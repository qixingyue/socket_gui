...
<div id="rightFrame">  
<div id="wrap">
<div class="pageTitle">设置比对任务</div>
  <?php echo form_open(site_url("compare/create"));?>
    <table class="border setCompareTable">
      ...
      <tr>
        <td>任务名称</td>
        <td><input class="taskName" type="text" name="taskName" disabled="disabled"/></td>
      </tr>
      <tr>
...
<script type="text/javascript">
	...
	$(function(){
		...
		_n("taskName").val("<?php echo $name?>");
		_n("sourcedb[tns]").val(current.source_db.tns);
		_n("sourcedb[user]").val(current.source_db.db_user);
		_n("sourcedb[password]").val(current.source_db.db_password);
		_n("targetdb[tns]").val(current.target_db.tns);
		_n("targetdb[user]").val(current.target_db.db_user);
		_n("targetdb[password]").val(current.target_db.db_password);
		_n("sql_condition").val(current.sql_condition);
		var m = current.execute_plan.plan_type.split("|");
		_n("month").val(m[0]);
		_n("week").val(m[1]);
		_n("day").val(m[2]);
		var n = current.execute_plan.start_time.split(":");
		_n("hour").val(n[0]);
		_n("minute").val(n[1]);
	});
</script>
